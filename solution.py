...
def get_route(hostname):
    ...
    for ttl in range(1,MAX_HOPS):
        for tries in range(TRIES):
            ...
            ICMP_SocketType = getprotobyname("icmp")
            mySocket = socket(AF_INET, SOCK_RAW, ICMP_SocketType) #no socket.socket needed
            #Fill in end
            ...
            try:
                ...
                try: #try to fetch the hostname
                    #Fill in start
                    OutHostName = gethostbyaddr(addr[0])[0]
                    #Fill in end
                except herror:   #if the host does not provide a hostname
                    #Fill in start
                    OutHostName = "hostname not returnable"
                    #Fill in end

                if ICMP_Type == 11: #timeout
                    ...
                    tracelist1.append([str(ttl), "*", "Exceltion occured"])
                    #Fill in end
                break
            finally:
                mySocket.close()
